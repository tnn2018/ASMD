...
parser.add_argument('--schedule', type=int, nargs='+', default=[40, 70],
...
# Checkpoints
parser.add_argument('-c', '--checkpoint', default='path', type=str, metavar='PATH',
                    help='path to save checkpoint (default: checkpoint)')
parser.add_argument('--resume', default='./checkpoint_vgg_C100/checkpoint.pth.tar', type=str, metavar='PATH',   # ./checkpoint/checkpoint.pth.tar
...
parser.add_argument('--retrain', dest='retrain', default=0, help='wether retrain from a decoupled model, only valid when evaluation is on', action='store_true')
parser.add_argument('--finetune_epoch', default=100, type=int, metavar='N',help='number of total epochs to run')
...
#        logger.set_names(['Learning Rate', 'Train Loss', 'Valid Loss', 'Train Acc.', 'Valid Acc.'])
    ...
    if args.evaluate:
        ...
        for i, t in enumerate(thresholds):
            ...
            test_model, store_r = f_decouple(test_model, look_up_table, criterion=sigma_criterion(t), train=False, sign = True, prune_epoch= 5)
            print(' Done! test decoupled model')
            test_loss, test_acc = test(testloader, test_model, criterion, start_epoch, use_cuda)
            print(' Test Loss :  %.8f, Test Acc:  %.2f' % (test_loss, test_acc))
            acc[i] = test_acc
